...
from .models.controller import ABSENT, PRESENT, UNMENTIONED
...
class AsymmetricLoss(nn.Module):
    ...
    def forward(self, logits, targets, mask=None):
        ...
        loss=-(pos+neg)
        if mask is not None:
            loss=loss*mask.float(); return loss.sum()/mask.sum().clamp_min(1)
        return loss.mean()
...
def grounding_loss(finding_tokens, region_tokens, region_logits, region_targets,
                   temperature=.07, contrastive_weight=1.0, assignment_weight=1.0):
    """Contrastive finding-region alignment + explicit region-assignment CE."""
    ...
    if valid.any():
        l_contrast=F.cross_entropy(sim[valid],region_targets[valid])
        l_assign=F.cross_entropy(region_logits[valid],region_targets[valid])
    else:
        zero=sim.sum()*0
        l_contrast=zero; l_assign=zero
    total=contrastive_weight*l_contrast+assignment_weight*l_assign
    return total, {"contrastive":l_contrast,"assignment":l_assign,"similarity":sim}
...
def unsupported_finding_loss(state_logits, support_scores):
    p_present=torch.softmax(state_logits,-1)[...,PRESENT]
...
class AEGISLoss(nn.Module):
    ...
    def forward(self,out,batch):
        ...
        clinically_known=(states==PRESENT)|(states==ABSENT)
        y=(states==PRESENT).float()
        l_clin=self.asl(out['clin_logits'],y,clinically_known)

        l_grd,_=grounding_loss(
            out['finding_tokens'],out['anatomy_tokens'],out['region_logits'],batch['region_targets'],
            self.cfg.loss.grounding_contrastive_temperature,
            self.cfg.loss.grounding_contrastive_weight,
            self.cfg.loss.grounding_assignment_weight,
        )
