...
class ChestXrayPreprocessor:
    def __init__(self, image_size=384, percentile_low=0.5, percentile_high=99.5,
                 training=False, rotation_deg=5.0, translate_frac=0.03, contrast_jitter=0.10):
    ...
    def __call__(self, image: Image.Image) -> torch.Tensor:
        ...
        lo, hi=np.percentile(arr, [self.p_low, self.p_high])
        arr=np.clip(arr, lo, hi)
        mean, std=float(arr.mean()), float(arr.std())
        arr=(arr-mean)/(std+1e-6)
        amin, amax=float(arr.min()), float(arr.max())
        u8=((arr-amin)/(amax-amin+1e-6)*255).astype(np.uint8)
        ...
        if self.training:
            ...
            max_shift=int(self.translate_frac*self.image_size)
            translate=[random.randint(-max_shift,max_shift), random.randint(-max_shift,max_shift)]
            img=TF.affine(img, angle=angle, translate=translate, scale=1.0, shear=[0.0,0.0], interpolation=InterpolationMode.BILINEAR, fill=0)
