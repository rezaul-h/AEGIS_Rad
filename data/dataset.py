...
from ..models.controller import ABSENT, PRESENT, UNCERTAIN, UNMENTIONED

STATE_TO_ID={"absent":ABSENT,"present":PRESENT,"uncertain":UNCERTAIN,"unmentioned":UNMENTIONED}
...
class ReportDataset(Dataset):
    def __init__(self, manifest: str|Path, split: str, tokenizer, ontology, cfg, training=False):
        ...
        self.tokenizer=tokenizer; self.ontology=ontology; self.cfg=cfg
        self.transform=ChestXrayPreprocessor(
            image_size=cfg.image_size, percentile_low=cfg.percentile_low,
            percentile_high=cfg.percentile_high, training=training,
            rotation_deg=cfg.rotation_deg, translate_frac=cfg.translate_frac,
    ...
    def __getitem__(self, idx):
        ...
        lateral=torch.zeros_like(frontal) if lateral_missing else self.transform(Image.open(r['lateral_path']))
        ...
        state=torch.full((F,), UNMENTIONED, dtype=torch.long)
        ...
        for f in self.ontology.findings:
            if f.name in finding_states:
                raw=str(finding_states[f.name]).lower().strip()
                if raw not in STATE_TO_ID:
                    raise ValueError(f"Invalid semantic state {raw!r} for finding {f.name!r}")
                state[f.id]=STATE_TO_ID[raw]
            rt=region_targets.get(f.name)
            if isinstance(rt,list) and rt: rt=rt[0]
            if rt in self.ontology.region_to_id: region[f.id]=self.ontology.region_to_id[rt]
            if f.name in support_targets: support[f.id]=float(support_targets[f.name])
        return {
            ...
            'region_targets':region, 'support_targets':support, 'report_text':str(r['report'])
        }

def collate_reports(batch, pad_id: int):
    maxlen=max(x['report_ids'].numel() for x in batch); B=len(batch)
    ids=torch.full((B,maxlen),pad_id,dtype=torch.long); mask=torch.zeros((B,maxlen),dtype=torch.bool)
    for i,x in enumerate(batch):
        n=x['report_ids'].numel(); ids[i,:n]=x['report_ids']; mask[i,:n]=True
    return {
        'study_id':[x['study_id'] for x in batch], 'frontal':torch.stack([x['frontal'] for x in batch]),
