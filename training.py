...
class Trainer:
    def __init__(self,model,cfg,tokenizer,device,output_dir):
        ...
        self.criterion=AEGISLoss(cfg,tokenizer.pad_id); self.best=float('inf'); self.bad=0
    def _move(self,b): return {k:(v.to(self.device) if torch.is_tensor(v) else v) for k,v in b.items()}
    def _lr(self,epoch):
        ...
        return self.cfg.training.lr*.5*(1+math.cos(math.pi*t))
    def run_epoch(self,loader,train=True):
        ...
        for b in tqdm(loader,leave=False):
            ...
            with torch.set_grad_enabled(train),torch.autocast(device_type=self.device.type,enabled=self.cfg.training.amp and self.device.type=='cuda'):
                out=self.model(b['frontal'],b['lateral'],b['lateral_missing'],inp,mask); losses=self.criterion(out,b)
            if train:
                self.optim.zero_grad(set_to_none=True); self.scaler.scale(losses['loss']).backward(); self.scaler.unscale_(self.optim)
                torch.nn.utils.clip_grad_norm_(self.model.parameters(),self.cfg.training.grad_clip); self.scaler.step(self.optim); self.scaler.update()
    ...
    def fit(self,train_loader,val_loader):
        ...
        for epoch in range(self.cfg.training.epochs):
            ...
            if va['loss']<self.best:
                self.best=va['loss']; self.bad=0; torch.save({'model':self.model.state_dict(),'epoch':epoch+1},self.output/'best.pt')
            else:self.bad+=1
            print(row)
            if self.bad>=self.cfg.training.early_stopping_patience:break
        import json; (self.output/'history.json').write_text(json.dumps(history,indent=2))
