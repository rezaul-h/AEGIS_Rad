...
class AEGISRad(nn.Module):
    def __init__(self,cfg,ontology,edge_index,edge_type,region_coords):
        super().__init__(); m=cfg.model; self.cfg=cfg; self.ontology=ontology
        ...
        self.graph=FindingAnatomyGraph(m.anatomy_slots,m.num_findings,m.hidden_dim,m.graph_layers,m.graph_heads,m.dropout)
        ...
        self.controller=FindingStateController(m.hidden_dim,m.num_findings,m.anatomy_slots,m.num_semantic_states,
                                               cfg.controller.tau_e,cfg.controller.tau_con,cfg.controller.tau_suppress)
        self.register_buffer('edge_index',edge_index,persistent=False); self.register_buffer('edge_type',edge_type,persistent=False)
        self.register_buffer('region_coords',region_coords,persistent=False); self.clin_head=nn.Linear(m.hidden_dim,m.num_findings)
    def encode(self,frontal,lateral,lateral_missing):
        f=self.encoder(frontal); l=self.encoder(lateral); study=self.multiview(f,l,lateral_missing)
        ...
        controller=self.controller(finding_tokens); pooled=finding_tokens.mean(1); clin_logits=self.clin_head(pooled)
    ...
    def forward(self,frontal,lateral,lateral_missing,input_ids,pad_mask=None):
        ...
        logits,dec=self.decoder(input_ids,enc['study_tokens'],enc['anatomy_tokens'],pad_mask,enc['anatomy_uncertainty'])
