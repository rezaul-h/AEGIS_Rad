...
class ConvVisionEncoder(nn.Module):
    """Dependency-free fallback for smoke tests. Use HFVisionEncoder for manuscript-style RAD-DINO."""
    def __init__(self, in_channels=1, hidden_dim=768):
        super().__init__()
        dims=[64,128,256,hidden_dim]
        layers=[]; c=in_channels
        for d in dims:
            layers += [nn.Conv2d(c,d,3,2,1), nn.GroupNorm(min(32,d),d), nn.GELU()]
            c=d
    ...
    def forward(self,x):
        z=self.net(x)
        patches=z.flatten(2).transpose(1,2)
        cls=patches.mean(1,keepdim=True)+self.cls
...
class HFVisionEncoder(nn.Module):
    def __init__(self, model_name='microsoft/rad-dino', revision=None, hidden_dim=768, in_channels=1):
        ...
        try:
            ...
            raise ImportError("Install `pip install -e '.[rad_dino]'` to use encoder_backend=hf") from e
    ...
    def forward(self,x):
        ...
        if tokens is None:
            raise RuntimeError('Vision model did not return last_hidden_state')
        return self.proj(tokens)

def build_vision_encoder(cfg):
    if cfg.encoder_backend.lower()=='hf':
        return HFVisionEncoder(cfg.encoder_name,cfg.encoder_revision,cfg.hidden_dim,cfg.in_channels)
    return ConvVisionEncoder(cfg.in_channels,cfg.hidden_dim)
