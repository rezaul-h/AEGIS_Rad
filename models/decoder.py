...
class ConfidenceGatedDecoder(nn.Module):
    ...
    def forward(self,input_ids,study_tokens,anatomy_tokens,pad_mask=None):
        B,T=input_ids.shape
        ...
        c_a,_=self.anatomy_attn(h,anatomy_tokens,anatomy_tokens,need_weights=False)
        g=torch.sigmoid(self.gate(torch.cat([h,c_v,c_a],-1)))
        c=g*c_a+(1-g)*c_v
        z=self.norm(h+self.context_proj(torch.cat([h,c],-1)))
        return self.lm_head(z), {'gate':g,'global_context':c_v,'anatomy_context':c_a,'hidden':h,'fused_context':c}
